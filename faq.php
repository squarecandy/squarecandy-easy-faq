...
function determine_is_faq()
{
	...
	foreach ($wp_query->posts as $eachpost) {
		...
		if (is_int($pos) || $posa!==false || $faq_checkbox=='yes' || $faq_shortcode_checkbox == 'yes') { //if this post needs the faq animations,
			$loadfaq = 1;
		}												//set this variable so we can add the animations
	}
	if ($loadfaq == 1) {
...
<p>If you chose to have the drop down effect applied only to certain sections of your content, just wrap each section of content that you want to have the effect with the shortcodes [accordion_start] and [accordion_end].</p>
...
<p>If you clicked the second checkbox to <strong>apply these effects only to certain portions of the page</strong>, you need to enter some shortcodes around your content. Type [accordion_start] in the post editor before the content that you want to have the effect, and [accordion_end] after the content.</p>
...
function make_faq_check_box($post)
{
	...
	<p><input name="make_faq_shortcode" type="checkbox" value="yes" <?php echo $checked_shortcode; ?> ></input> Check this box to apply the drop-down effect only to sections on the page wrapped in the shortcodes [accordion_start] and [accordion_end]</p>
...
add_shortcode('accordion_start', 'bg_faq_shortcode_start');
add_shortcode('accordion_end', 'bg_faq_shortcode_end');
